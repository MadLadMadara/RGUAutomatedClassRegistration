...
class DocumentParser {
    ...
    static function parseFromFile($filename, $mimetype = null) {
        if (!is_readable($filename)) {
            throw new Exception('Failed to read file: cannot read file %s');
        }
        if (!$mimetype) {
        ...
        } else if ($mimetype === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' || $mimetype === 'application/octet-stream') {
            return self::parseZipped($filename, 'word/document.xml');
        } else if ($mimetype === 'application/vnd.oasis.opendocument.text') {
            return self::parseZipped($filename, 'content.xml');
        } else {
            throw new Exception('Failed to read file: unknown mimetype %s');
        }
    }
    /**
